ISLaboratoryTable = ISBuildingObject:derive("ISBuildingObject");

function ISLaboratoryTable:create(x, y, z, north, sprite)
	...
	self.javaObject:setName("Laboratory Table")
	self.javaObject:setMaxHealth(self:getHealth());
	self.javaObject:setHealth(self.javaObject:getMaxHealth());
	self.javaObject:setBreakSound("BreakObject");

	buildUtil.consumeMaterial(self);
	self.sq:AddSpecialObject(thumpable);
	self.javaObject:transmitCompleteItemToServer();
end

function ISLaboratoryTable:removeFromGround(square)
...
function ISLaboratoryTable:new()
	local o = {};
	setmetatable(o, self);
	self.__index = self;
	o:init();

	o.name = getText(LaboratoryTableMenu.NameID);
	o:setSprite(LaboratoryTableMenu.sprites.west[0], LaboratoryTableMenu.sprites.west[1]);
	o:setNorthSprite(LaboratoryTableMenu.sprites.north[0], LaboratoryTableMenu.sprites.north[1]);
	o:setEastSprite(LaboratoryTableMenu.sprites.east[0], LaboratoryTableMenu.sprites.east[1])
	o:setSouthSprite(LaboratoryTableMenu.sprites.south[0], LaboratoryTableMenu.sprites.south[1])
...
function ISLaboratoryTable:isValid(square)
...
function ISLaboratoryTable:render(x, y, z, square)
...
function ISLaboratoryTable:getSquare2Pos(square, north)
	local x = square:getX()
	local y = square:getY()
	local z = square:getZ()
	if north then
...
function ISLaboratoryTable:getSquare2(square, north)
	local x, y, z = self:getSquare2Pos(square, north)
	return getCell():getGridSquare(x, y, z)
end

function ISLaboratoryTable:walkTo(x, y, z)
